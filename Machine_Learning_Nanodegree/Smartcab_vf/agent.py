...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=True):
        ...
        if testing == True:
            ...
            #self.epsilon = self.epsilon * 0.93
            #self.epsilon = self.epsilon - 0.05
            #self.epsilon = (self.alpha ** self.counter)
            #self.epsilon = 1.0/(self.counter**2)
            #self.epsilon = (math.e**(self.alpha*self.counter*-1))
            #self.epsilon = (math.cos(self.alpha*self.counter))
            self.epsilon = (self.epsilon * (0.99993**(2+self.counter)))
            #self.epsilon = 1 - (1/(1+math.exp(-self.alpha*(self.counter))))
            # self.alpha = 0.5
            self.counter +=1
            #bind epsilon from 0 to 1
            if self.epsilon > 1.0:
                self.epsilon = 1.0
            elif self.epsilon < 0.0:
                self.epsilon = 0.0
            
            #bind alpha from 0 to 1
            #if self.alpha > 1.0:
            #    self.alpha = 1.0
            #elif self.alpha < 0.0:
            #    self.alpha = 0.0
...
def run():
    ...
    agent = env.create_agent(LearningAgent,alpha=0.1)
